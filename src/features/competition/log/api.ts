...
export const competitionLogApi = {
  list: (params?: { page?: number; size?: number }) =>
    apiClient
      .get('journal/competition-logs', {
        searchParams: { page: params?.page ?? 0, size: params?.size ?? 20, sort: 'eventDate,desc' },
      })
      .json<Page<CompetitionLog>>(),

  get: (id: number) => apiClient.get(`journal/competition-logs/${id}`).json<CompetitionLog>(),

  create: (data: CreateCompetitionLogRequest) =>
    apiClient.post('journal/competition-logs', { json: data }).json<CompetitionLog>(),
